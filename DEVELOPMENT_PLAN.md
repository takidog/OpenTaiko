...
## 15. 後續需求：網頁瀏覽、遊玩控制與結算切歌改善（分階段實作中）
...
**實作狀態：** 已完成。前端使用 IntersectionObserver 自動載入下一批資料；篩選改變時會中止舊請求並以 generation 隔離過期結果，同時避免重複 ID、支援載入／完成／錯誤狀態及點擊重試。
...
**實作狀態：** 已完成。歌曲標題限制兩行、作者／副標題限制一行，並對目前選曲、最近遊玩、badge、播放清單對話框及 grid/flex 子元素加入縮排與溢出防護；完整曲名及作者可由 title tooltip 查看。

### 15.2.1 曲庫完成後同步刷新與空分類修正

**新增問題：** 網頁可能在 OpenTaiko 尚未完成歌曲索引時先取得空分類；使用歌曲快取時，歌曲也可能在繼承父分類前就被複製進遠端索引。

**實作狀態：** 已完成。快取歌曲先套用父分類再加入 `CSongDict`；曲庫建立完成後發送 SSE `catalog` 事件。前端以事件為主、health 輪詢為保險，完成後重新載入分類、歌曲、播放清單、最近遊玩及目前狀態。來源確實沒有分類的歌曲統一歸入 `OTHER`。
...
**實作狀態：** 第一版已完成。`GET /state` 與 SSE 已提供播放狀態、歌曲資料、時間、進度及控制能力；前端已加入固定進度卡。自動 API 測試已完成，仍需正式環境實際遊玩驗收時間準確度。
...
**實作狀態：** 第一版已完成。新增 `/gameplay/exit` 與 `/gameplay/retry`，前端依 capability flags 啟用大型按鈕；遠端退出會將未完成歷史標記為 `aborted`。仍需正式環境驗收淡出、暫停與不同譜面模式。
...
**實作狀態：** 第一版已完成。結算階段允許 select、play 與 restart；離場前執行冪等的成績／歷史後處理，再初始化標準 SongSelect 並執行原命令。仍需正式環境驗收一般、Tower 與 Dan 結算流程。

### 15.5.1 結算返回、Preview、自動返回與音訊清理

**新增需求與 Bug：** 結算畫面需要由網頁立即返回選歌，或依網頁設定的秒數自動返回；結算時選歌曾因離場未完整停止音訊，造成結算 BGM 與歌曲同時播放。結算時的遊戲 Preview 應先安全退出結算再執行，最愛操作則不應受遊玩／結算 stage 限制。

**實作狀態：** 已完成。新增 `/results/exit`，並讓 Preview 在 Results 使用與 select／play 相同的安全轉場。結算舞台離場會集中停止進場、循環、AI、Dan、Tower BGM 與結果語音後再釋放資源。Favorite 命令可在任何 stage 經主執行緒執行。網頁提供 0–3600 秒的自動返回設定（0 為關閉），以瀏覽器 localStorage 保存並顯示倒數；手動選歌、Preview 或播放會取消倒數，避免重複命令。
